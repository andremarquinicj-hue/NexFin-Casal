...
import { createCardPurchase, createFinancialTransaction, createItem } from "@/lib/firestore";
...
export default function QuickEntry({onClose}:{onClose:()=>void}){
 ...
 return <div className="modal-backdrop" onMouseDown={onClose}><form className="modal" onSubmit={save} onMouseDown={e=>e.stopPropagation()}>
  <div className="modal-head"><div><h2>Novo lançamento</h2><p>{cardMode?"Compras no cartão entram na fatura automaticamente.":"Planeje agora ou registre um valor já realizado."}</p></div><button type="button" onClick={onClose}><X/></button></div>
  ...
  <button className="primary-btn wide" disabled={saving}>{saving?"Salvando...":cardMode?"Salvar compra no cartão":execution==="actual"?"Salvar e atualizar saldo":"Salvar provisionamento"}</button>
 </form></div>
}
